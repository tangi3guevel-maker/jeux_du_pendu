...
int choisir_N_erreurs(){
    srand(time(NULL));
    return(rand()%(MAX_ERREUR-MIN_ERREUR+1)+MIN_ERREUR);
}

int main(void){
    char* liste_mots[] = { "bonjour", "maison", "papillon", "artisan","ruelle", "ordinateur", "chocolat", "fromage", "jardin", "etoile","nuages", "cyclone", "robotique", "escargot", "chameau", "probleme","terrasse", "bonsoir", "biscuit", "catastrophe" };
